...
func TestProgramHash(t *testing.T) {
	tests := []struct {
		...
		{
			url:  "https://alpha4.starknet.io/feeder_gateway/get_class_by_hash?classHash=0x010455c752b86932ce552f2b0fe81a880746649b9aee7e0d842bf3f52378f9f8",
			want: hexToFelt("0x1e87d79be8c8146494b5c54318f7d194481c3959752659a1e1bce158649a670"),
		},
		{
			url:  "https://alpha4.starknet.io/feeder_gateway/get_class_by_hash?classHash=0x056b96c1d1bbfa01af44b465763d1b71150fa00c6c9d54c3947f57e979ff68c3",
			want: hexToFelt("0x359145fc6207854bfbbeadae4c6e289024400a5af87090ed18073200fef6213"),
		},
		{
			url:  "https://alpha4.starknet.io/feeder_gateway/get_class_by_hash?classHash=0x0079e2d211e70594e687f9f788f71302e6eecb61d98efce48fbe8514948c8118",
	...
	for _, tt := range tests {
		t.Run("ProgramHash", func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("unexpected error while fetching contract definition: %s", err)
			}

			body, err := io.ReadAll(resp.Body)
			if err != nil {
				t.Fatalf("unexpected error while reading contract definition: %s", err)
			}

			var contractDefinition ContractDefinition
			if err := json.Unmarshal(body, &contractDefinition); err != nil {
				t.Fatalf("unexpected error while unmarshaling contract definition: %s", err)
			}

			programHash, err := ProgramHash(body)
			if err != nil {
