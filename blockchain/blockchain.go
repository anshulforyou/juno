...
// BlockDbKey creates appends block number and hash to create a db key.
...
func NewBlockchain(database db.DB, network utils.Network) (*Blockchain, error) {
	...
	if err = gob.NewDecoder(bytes.NewReader(headBlockBin)).Decode(headBlock); err != nil {
		return nil, err
	}
...
func (b *Blockchain) Store(block *core.Block, stateUpdate *core.StateUpdate) error {
	...
	if err = gob.NewEncoder(buffer).Encode(block); err != nil {
		return err
	}
	blockBinary := append([]byte{}, buffer.Bytes()...)
